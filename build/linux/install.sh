...
for directory in $BIN_PATH $LOG_PATH $CONFIG_PATH $CERTS_PATH $USERS_PATH $CERTDIR_TRUSTEDCAS $CERTDIR_TRUSTEDJWTCERTS $CERTDIR_SIGNINGKEYS $CERTDIR_TLS $CERTDIR_APSJWT $KEYS_PATH $KEYS_TRANSFER_POLICY_PATH $SAML_CERTS_PATH $TPM_IDENTITY_CERTS_PATH; do
    mkdir -p $directory
    if [ $? -ne 0 ]; then
